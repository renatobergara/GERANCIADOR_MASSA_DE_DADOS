...
public class XLSBeans {
	...
	/**
	 * Loads one sheet as the single object from the Excel file. using default
	 * JExcel API.
	 *
...
//		jxl.Sheet jxlSheet = null;
	...
	public <P> P load(InputStream xlsIn, InputStream xmlIn, Class<P> clazz, String bookType) throws XLSBeansException {
		try {
			...
			Sheet sheet = getSheetAnnotation(reader, clazz);
			if (sheet == null) {
				throw new XLSBeansException("Can't find @Sheet.");
			}
			try {
	...
	/**
	 * Loads sheets as multiple objects from the Excel file. using default JExcel
	 * API.
	 *
	 * @param <P>   the type parameter for the object type
	 * @param xlsIn the InputStream of the Excel file
	...
	public <P> P[] loadMultiple(InputStream xlsIn, InputStream xmlIn, Class<P> clazz, String bookType)
			throws XLSBeansException {
		try {
			XMLInfo info = null;
			if (xmlIn != null) {
	...
	/**
	 * Loads sheets as multiple type objects from the Excel file. using default
	 * JExcel API.
	 *
	 * @param xlsIn   the InputStream of the Excel file
	 ...
	 * @throws XLSBeansException
	 */
	public Object[] loadMultiple(InputStream xlsIn, InputStream xmlIn, Class<?>[] classes, String bookType)
			throws XLSBeansException {
		try {
			XMLInfo info = null;
			if (xmlIn != null) {
	...
	//
	// Esses dois métodos são ganchos, para podermos sobrescrever o comportamento de
	// como são criados os repositórios e de onde são pegos as anotações de Sheet
	public <P> P newInstance(Class<P> clazz) throws InstantiationException, IllegalAccessException {
		return clazz.newInstance();
	}

	public <P> Sheet getSheetAnnotation(AnnotationReader reader, Class<P> clazz) throws Exception {
		return reader.getAnnotation(clazz, Sheet.class);
	}
}
